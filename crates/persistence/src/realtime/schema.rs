...
fn realtime_table_statements(user_prefix: &str) -> Vec<String> {
    vec![
        ...
        format!("CREATE TABLE IF NOT EXISTS {user_prefix}_tracked_nonfriends (user_id TEXT PRIMARY KEY, display_name TEXT NOT NULL DEFAULT '', added_at TEXT NOT NULL DEFAULT '')"),
        format!("CREATE INDEX IF NOT EXISTS {user_prefix}_tracked_nonfriends_added_idx ON {user_prefix}_tracked_nonfriends (added_at DESC, user_id ASC)"),
