...
1. **直接实施**：Feed 图片拖放上传到 Gallery；Jirai 默认开发者群自动加入（带现有设置页开关、可见结果和失败提示）。
2. **先建立可实施规格**：非好友追踪、手动关系、资料补全自动工作流与历史补线。它们是关系推荐的前置依赖，不能再被笼统标记为“以后再说”。

多账户暂不实现；单独进行 Rust-first 架构 RFC。状态历史分布保留：其初版来自 Copilot 提交 `137abf6f`，但后续由 FuLu糖福禄在 `cf942bd0` 重构为按时长统计并支持自我追踪，符合“作者本人或其合入贡献”的审计范围。
...
这样既能满足“抓取非好友数据”，又不污染好友关系的事实模型。

### 2. 手动关系本来就应是独立表和显示层

**同意。** 旧版确实用独立的 `{prefix}_manual_relations_MANUEL` 表保存手动边；
问题不是“要不要独立”，而是新实现不能为图方便把这类边写进当前
`*_mutual_graph_links` 观测快照。新表 + 图上的显式覆盖层正是旧行为在当前架构中的等价、可维护迁移。
...
推荐算法读取 `F`、`G`，并为完整 Jirai 行为使用 `T` 与 `M`。手动关系不是
...
- 旧的 5 秒 `OnPlayerJoined`/`OnPlayerLeft` 写入位于 Previous Instances 对话框的**手动操作**，并以当前用户最近进入该实例的时间为起点；它不是启动自动轮询；
- 因此，当前或旧版都无法从 VRChat 的“当前在线”资料准确倒推出 13:00 的真实上线时间。Activity/Online Overlap 没有起始观测时只能忽略该段，或从最早已观测到的时刻起计算下界。

我不同意把未知的 13:00 当普通真实记录写入；这会把下界伪装为事实。但同意“17:00 已确认在线”是有价值的补线。
...
详情、导出、推荐计算可显示或选择排除其来源。这同时满足数据不继续缺失和可追溯性。
...
明确跳过。完成非好友追踪后，自动工作流再从好友范围扩展到追踪名单。
