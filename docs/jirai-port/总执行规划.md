...
| 编号 | 任务 | 直接依赖 | 可并行 | 本阶段性质 |
| --- | --- | --- | --- | --- |
| 01 | 功能来源审计修订 | 无 | 02、03、10 | 文档 |
| 02 | Feed 图片拖放到 Gallery | 无 | 01、03、10 | 实现 |
| 03 | 默认开发者群自动加入 | 无 | 01、02、10 | 实现 |
| 04 | workflow UI 与调度目录 | 01 | 05、06、07 | 实现/架构 |
| 05 | 启动 Online 补线 | 04 | 06、07 | 实现 |
| 06 | 已跟踪非好友 | 04 | 05、07 | 实现 |
| 07 | 手动关系独立层 | 01 | 05、06 | 实现 |
| 08 | 自动资料抓取 | 04、05；扩展部分依赖 06 | 07 | 实现 |
| 09 | 关系推荐 | 06、07、08 | 无 | 实现 |
| 10 | 多账户进程隔离 RFC | 无 | 全部 | 仅调研 |
| 11 | 自动跟随好友实例 | 无 | 09、10 | 实现（低验证优先级） |
...
gantt
    ...
    section 并行启动
    01 来源审计修订                 :crit, t01, 2026-01-01, 1d
    02 Feed→Gallery 拖放             :crit, t02, 2026-01-01, 2d
    03 默认开发者群自动加入          :t03, 2026-01-01, 1d
...
1. 每个任务都先读其 `执行任务/` 下的独立 brief，再委派子代理；不得只给一句“移植功能”。
2. 每个子代理只改其任务明确列出的文件/边界；遇到上游数据表或 runtime 核心改动必须停下报告。
3. 主对话负责检查旧代码复用点、命名兼容、依赖是否满足、测试和视觉证据。
4. 每项完成后独立提交；不把未验证的复杂数据库任务与简单 UI 任务混合提交。
5. 未经用户新的明确要求，不执行 09、10 的实现，也不启动 Previous Instances 手动补实例记录。
